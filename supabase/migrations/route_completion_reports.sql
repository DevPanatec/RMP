-- ============================================
-- MÓDULO DE REPORTES DE RUTAS COMPLETADAS
-- ============================================
...
-- Policy: Permitir actualización solo del propio resumen (por conductor)
CREATE POLICY "Permitir actualización de resúmenes propios"
  ON resumen_rutas_completadas
  FOR UPDATE
  TO authenticated
  USING (conductor_nombre = current_user OR current_user IN (
    SELECT nombre FROM empleados WHERE cargo IN ('admin', 'supervisor')
  ))
  WITH CHECK (conductor_nombre = current_user OR current_user IN (
    SELECT nombre FROM empleados WHERE cargo IN ('admin', 'supervisor')
  ));

-- Policy: Solo admins pueden eliminar
CREATE POLICY "Solo admins pueden eliminar resúmenes"
  ON resumen_rutas_completadas
  FOR DELETE
  TO authenticated
  USING (current_user IN (
    SELECT nombre FROM empleados WHERE cargo = 'admin'
  ));

-- Grant permissions
GRANT SELECT, INSERT ON resumen_rutas_completadas TO authenticated;
GRANT UPDATE, DELETE ON resumen_rutas_completadas TO authenticated;
